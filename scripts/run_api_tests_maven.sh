#!/bin/bash
# Script to run API tests using Maven
...
cleanup() {
    if [[ -n "$APP_PID" ]]; then
        ...
        # This is more reliable than killing the Maven wrapper
        ...
        # Also try to kill the Maven process tree
        if kill -0 "$APP_PID" 2>/dev/null; then
...
echo -e "${GREEN}=== API Test Runner (Maven) ===${NC}"
echo ""
...
if [[ "$START_APP_LOCALLY" == "true" ]]; then
    ...
    if curl -s -f "$API_BASE_URL/actuator/health" > /dev/null 2>&1; then
        ...
        # Start spring-boot:run in background and capture PID
...
TEST_EXIT_CODE=0
# Run Maven with apiTest profile and pass system properties
if ./mvnw test -PapiTest \
    -Dapi.base.url="$API_BASE_URL" \
    -Dgrpc.host="$GRPC_HOST" \
    -Dgrpc.port="$GRPC_PORT"; then
    echo ""
